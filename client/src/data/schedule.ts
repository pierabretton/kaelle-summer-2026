...
function block(
  ...
  return { start, end, activity, label, icon, note };
}

// ── Week 1: July 13–17 — Summer Camp ─────────────────────────
const week1Days: DaySchedule[] = [
  day("2025-07-13", "Mon · Jul 13", [
    block("09:00", "15:30", "summer-camp", "Summer Camp", "☀️"),
...
// ── Weeks 3–5: Aug 4–22 — Calmar Kids (3 weeks) ──────────────
function calmarWeek(
  dates: Array<[string, string]>
): DaySchedule[] {
  return dates.map(([date, label]) =>
    day(date, label, [
      block("08:45", "14:00", "calmar-kids", "Calmar Kids Summer Camp", "🌿"),
      block("14:00", "18:00", "purple-dragon", "Purple Dragon with Nanny", "🐉"),
    ])
  );
}

const week3Days = calmarWeek([
...
// ── Majorca Holiday: Aug 20 – Oct 31 ─────────────────────────
// Represented as a single special block (not week-by-week)
export const MAJORCA_BLOCK = {
  start: "2025-08-20",
  end: "2025-10-31",
  activity: "majorca" as ActivityType,
  label: "Holiday in Majorca! 🌴",
